...
public class Ejercicio4_Agregacion
{
    public void Ejecutar()
    {
        ...
        {
            foreach (var i in producto.Productos )
            {
                
                if(!(productos.Contains(i.Nombre)))
                {
                    productos.Add(i.Nombre);
                    Count++;
        ...
        Console.WriteLine($"Total de productos en todos los pedidos: {Count}");
...
public class Ejercicio5_MultiplesCondiciones
{
    public void Ejecutar()
    {
        ...
        {
            if (estudiante.Calificaciones.Any(calificacion => calificacion.Nota >= 4.5))
            {
...
public class Ejercicio7_Promedios
{
    public void Ejecutar()
    {
        ...
        foreach (var estudiante in estudiantes)
        {
            ...
            double promedio = estudiante.Calificaciones.Count > 0
                ? suma / estudiante.Calificaciones.Count
                : 0;
...
// ============================================
// Enunciado: Contar cuántos usuarios tienen al menos un rol
// cuyo nombre contenga "Admin" o "Moderador".
// 
// Debes usar: foreach + Any() con Contains()
// 
// Salida esperada:
// Usuarios con roles administrativos: 3

public class Ejercicio9_ConteoCondicional
{
    public void Ejecutar()
    {
        List<Usuario> usuarios = DatosPrueba.ObtenerUsuarios();

        Console.WriteLine("=== EJERCICIO 9: Conteo Condicional ===");

        int cantidadAdministrativos = 0;

        foreach (var usuario in usuarios)
        {
            if (usuario.Roles != null && usuario.Roles.Any(rol => rol.Nombre.Contains("Admin") || rol.Nombre.Contains("Moderador")))
            {
                cantidadAdministrativos++;
            }
        }

        Console.WriteLine($"Usuarios con roles administrativos: {cantidadAdministrativos}");
...
// ============================================
// EJERCICIO 10: Proyectar Datos Complejos
// ============================================
// Enunciado: Recorrer todos los pedidos y construir una lista plana
// con los nombres de los productos, sin repetir ninguno.
// 
// Debes usar: foreach anidado + HashSet
// 
// Salida esperada:
// Todos los productos únicos:
// - Laptop
// - Mouse
// - Teclado
// - Monitor
...
public class Ejercicio10_Proyeccion
{
    public void Ejecutar()
    {
        List<Pedido> pedidos = DatosPrueba.ObtenerPedidos();

        Console.WriteLine("=== EJERCICIO 10: Proyección de Datos Complejos ===");
        Console.WriteLine("Todos los productos únicos:");

        HashSet<string> productosUnicos = new HashSet<string>();
        List<string> ordenProductos = new List<string>();

        foreach (var pedido in pedidos)
        {
            foreach (var producto in pedido.Productos)
            {
                if (productosUnicos.Add(producto.Nombre))
                {
                    ordenProductos.Add(producto.Nombre);
                }
            }
        }

        foreach (var producto in ordenProductos)
        {
...
public class Program
{
    public static void Main()
    {
        ...
        Ejercicio9_ConteoCondicional ej9 = new Ejercicio9_ConteoCondicional();
        ej9.Ejecutar();

        Ejercicio10_Proyeccion ej10 = new Ejercicio10_Proyeccion();
        ej10.Ejecutar();
    }
}